...
public:
    explicit CRCommanderRos2(const std::string &ip, bool verbose = false);
    ...
    // Modbus 控制函数
    bool modbusClose(int index);
    bool modbusRTUCreate(int slave_id = 1, int baud = 115200, 
                        const std::string& parity = "N", int data_bit = 8, int stop_bit = 1);
    bool setHoldRegs(int index, int addr, int count, const std::string& val_tab, const std::string& val_type = "U16");
    bool getHoldRegs(int index, int addr, int count, const std::string& val_type, std::string& result);
